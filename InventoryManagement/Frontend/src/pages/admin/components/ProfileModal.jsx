...
const ProfileModal = ({ onClose }) => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      console.log("Using stored user data:", storedUser); // Debug the fallback data
      if (storedUser) {
  ...
  const getRoleName = (privileges) => {
    console.log("Getting role name for privileges:", privileges, typeof privileges);
  ...
  const roleColor = () => {
    const privNumber = Number(userData?.privileges);
    
    switch (privNumber) {
      case 1: return 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200'; // Worker
  ...
  console.log("Privileges:", userData.privileges, "Type:", typeof userData.privileges);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-0 w-[90%] max-w-3xl max-h-[90vh] overflow-hidden shadow-2xl relative">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-purple-800 p-8 flex items-center space-x-6">
          <div className="w-24 h-24 rounded-full bg-white flex items-center justify-center text-purple-700 text-4xl font-bold shadow-lg ring-4 ring-white">
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-lg font-medium dark:text-white">{userData?.username || 'Not available'}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Email</h4>
                    <p className="text-lg font-medium dark:text-white overflow-hidden text-ellipsis">{userData?.email || 'Not available'}</p>
                  </div>
                  <div>
              ...
              <form onSubmit={handlePasswordChange} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500 dark:bg-gray-600 dark:border-gray-500 dark:text-white"
                    required
                  />
                </div>
                ...
                <div className="pt-4">
                  <button
                    ...
                    className="w-full bg-purple-600 hover:bg-purple-700 text-black py-2 px-4 rounded-lg transition-colors flex items-center justify-center"
                  >
                    {isSubmitting ? (
                      <>
                        <div className="animate-spin rounded-full h-4 w-4 border-2 border-purple-200 border-t-transparent mr-2"></div>
                        Processing...
                      </>
                    ) : (
                      'Change Password'
