...
const Orders = () => {
  ...
  const downloadReport = async (orderId) => {
    try {
      setLoading(true);
      const response = await fetchWithAuth(`/orders/${orderId}/report`, {
        responseType: 'blob'
      });
      ...
      let filename = `Order_${orderId}_Report.pdf`;
  ...
  const viewReport = async (orderId) => {
    try {
      ...
      console.log(`Attempting to fetch report for order ID: ${orderId}`);
      console.log(`Order details:`, orderExists);
      
      const response = await fetchWithAuth(`/orders/${orderId}/report`, {
        responseType: 'blob'
      });
      
      // Since our enhanced fetchWithAuth now returns a Blob directly for responseType: 'blob'
      const url = window.URL.createObjectURL(response);
      setSelectedReport(url);
      setShowPreview(true);
    } catch (error) {
      console.error('Error viewing report:', error);
      console.error(`Failed to fetch report with ID: ${orderId}`);
      console.error(`API endpoint used: /orders/${orderId}/report`);
  ...
  // Modified to show a note about inventory manager approval
  const updateOrderStatus = async (orderId, status) => {
    try {
      ...
      // Show feedback message based on status
      if (status === 'completed') {
        alert('Order is now marked as completed. Note that for inventory updates to take effect, this action still requires approval from an inventory manager.');
      } else {
        alert(`Order status updated to ${status}. This is a local status change and requires inventory manager approval to be finalized.`);
      }
    } catch (error) {
  ...
  // New method to set an order to processing status
  const setOrderToProcessing = async (orderId) => {
    try {
      if (!orderId) return;
      ...
      console.log(`Order ${orderId} set to processing status`);
    } catch (error) {
      console.error('Error setting order to processing:', error);
    }
  };
  ...
  // Updated to handle order ID from form submission
  const handleOrderComplete = async (orderId) => {
    if (orderId) {
      await setOrderToProcessing(orderId);
    }
    await fetchOrders();
  ...
  // Same as above but for purchase orders
  const handlePurchaseOrderComplete = async (orderId) => {
    if (orderId) {
      await setOrderToProcessing(orderId);
    }
    await fetchOrders();
    // Switch to purchase orders tab when purchase order is created
    ...
    <div className="min-h-screen w-full p-6">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 mb-6">
          <h3 className="font-medium text-blue-800 flex items-center">
            <i className="fas fa-info-circle mr-2"></i> Order Approval Process
          </h3>
          <p className="text-sm text-blue-600 mt-1">
            Newly created orders are set to "processing" status automatically and must be approved by an inventory manager 
            before they affect inventory levels. The inventory manager will review and either approve or reject your orders.
          </p>
        </div>
                  ...
                  <tr key={order.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {(order.status === 'processing' || order.status === 'pending') && (
                        <p className="text-xs text-gray-500 mt-1">Awaiting inventory manager approval</p>
                      )}
                    </td>
      ...
      {showPurchaseForm && (
        <PurchaseOrderForm 
          ...
          onOrderComplete={handlePurchaseOrderComplete}
        />
      )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-80">
            ...
            <div className="mb-3">
              <p className="text-sm text-amber-600 bg-amber-50 p-3 rounded-lg border border-amber-200">
                ...
                Note: Changing status here is a local update. Final approval from an inventory manager is still required for inventory adjustments.
              </p>
            </div>
